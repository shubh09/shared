...
int bfs(int b,int e, vvi G)
{
    ...
    while (!q.empty())
    {
        ...
        int cur=curp.first;
        int num=curp.second;
        //assuming cur can't be e - check whether an element is e before pushing
        int i;
        FOR(i,0,n)
        {
            bool isEdge=(G[cur][i]==1);
            if (isEdge)
            {
                if (i==e) return num+1;
                if (vis[i]==0)
    ...
    return -1;  //e is unreachable from b
}

class Egalitarianism {
	public:
	int maxDifference(vector <string> v, int d)
	{
            int n=sz(v);
            vvi arr(n,vi(n,0));
            int i,j;
            FOR(i,0,n) FOR(j,0,n) if (v[i][j]=='Y') arr[i][j]=1;
            int ans=0;
            int shortestPath;
            FOR(i,0,n)
            {
                FOR(j,0,n)
                {
                    if (i!=j)
                    {
                        shortestPath=bfs(i,j,arr);
                        if (shortestPath>0) ans=max(ans,shortestPath*d);
                        else ans=INF;
                    }
                }
            }
