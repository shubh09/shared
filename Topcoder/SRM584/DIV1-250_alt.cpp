...
class Egalitarianism {
	...
	{
            int n=sz(v);
            vvi shortestDistances(n,vi(n,1000000));
            int i,j,k;
            FOR(i,0,n) shortestDistances[i][i]=0;
            FOR(i,0,n) FOR(j,0,n) if (v[i][j]=='Y') shortestDistances[i][j]=1;
            FOR(k,0,n) FOR(i,0,n) FOR(j,0,n) shortestDistances[i][j]=min(shortestDistances[i][j],shortestDistances[i][k]+shortestDistances[k][j]);
//            FOR(i,0,n)
//            {
//                FOR(j,0,n) printf("%d ",shortestDistances[i][j]);
//                printf("\n");
//            }
            int maxShortestDistance=0;
            FOR(i,0,n) FOR(j,0,n) maxShortestDistance=max(maxShortestDistance,shortestDistances[i][j]);
            if (maxShortestDistance==1000000) return -1;
            return maxShortestDistance*d;
	}
